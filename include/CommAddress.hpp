...
public:
	 ...
	 */
	double value() const { return (double)IP.to_v4().to_ulong() + (port / 65536.0); }
	 ...
	 */
	bool operator<(const CommAddress & r) const { return value() < r.value(); }
	 ...
	 */
	std::string getIPString() const { return IP.to_string(); }
	...
	uint32_t getIPNum() const { return IP.to_v4().to_ulong(); }
