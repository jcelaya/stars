...
public:
	 ...
	 */
	boost::filesystem::path getWorkingPath() { return workingPath; }

	/**
	 * Sets the working path of the application.
	 */
	void setWorkingPath(boost::filesystem::path p) { workingPath = p; }

	/**
	 * Returns the maximum bandwidth for availability information updates.
	 */
	double getUpdateBandwidth() const { return updateBW; }

	/**
	 * Sets the maximum bandwidth for availability information updates.
	 */
	void setUpdateBandwidth(double bw) { updateBW = bw; }
	 ...
	 */
	void setStretchRatio(double sr) { stretchRatio = sr; }
	 ...
	 */
	uint16_t getUIPort() const { return uiPort; }

	/**
	 * Sets the port number for the user interface.
	 */
	void setUIPort(uint16_t p) { uiPort = p; }

	/**
	 * Returns the logging configuration string.
	 */
	const std::string & getLogConfig() const { return logString; }

	/**
	 * Sets the logging configuration string.
	 */
	void setLogConfig(const std::string & s) { logString = s; }

	/**
	 * Returns the number of retries of a failed submission.
	 */
	int getSubmitRetries() const { return submitRetries; }

	/**
	 * Sets the number of retries of a failed submission.
	 */
	void setSubmitRetries(int r) { submitRetries = r; }

	/**
	 * Returns the number of seconds between heartbeat signals from scheduler to submission nodes
	 */
	unsigned int getHeartbeat() const { return heartbeat; }

	/**
	 * Sets the number of seconds between heartbeat signals from scheduler to submission nodes
	 */
	void setHeartbeat(unsigned int h) { heartbeat = h; }

	/**
	 * Returns the working path of the application. It is used for relative paths.
	 */
	boost::filesystem::path getDatabasePath() { return dbPath; }

	/**
	 * Sets the working path of the application.
	 */
	void setDatabasePath(boost::filesystem::path p) { dbPath = p; }

	/**
	 * Returns the maximum relation between maximum and minimum stretch.
	 */
	unsigned int getAvailableMemory() const { return availMemory; }

	/**
	 * Sets the maximum relation between maximum and minimum stretch.
	 */
	void setAvailableMemory(unsigned int m) { availMemory = m; }

	/**
	 * Returns the maximum relation between maximum and minimum stretch.
	 */
	unsigned int getAvailableDisk() const { return availDisk; }

	/**
	 * Sets the maximum relation between maximum and minimum stretch.
	 */
	void setAvailableDisk(unsigned int d) { availDisk = d; }
