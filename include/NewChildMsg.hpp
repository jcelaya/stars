...
public:
	...
	virtual NewChildMsg * clone() const { return new NewChildMsg(*this); }
	 ...
	 */
	const CommAddress & getChild() const { return child; }

	/**
	 * Sets the address of the node that has been added.
	 * @param addr The node address.
	 */
	void setChild(const CommAddress & addr) { child = addr; }
	 ...
	 */
	bool replaces() const { return replace; }
	 ...
	 */
	void replaces(bool r) { replace = r; }

	// This is documented in BasicMsg
	void output(std::ostream& os) const {}

	// This is documented in BasicMsg
	std::string getName() const { return std::string("NewChildMsg"); }
};
