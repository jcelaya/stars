...
public:
	struct AppDesc {
		...
		double getDeadline(double S) const { return S * w + r; }
		void setStretch(double S) { d = getDeadline(S); }
		bool operator<(const AppDesc & rapp) const { return d < rapp.d || (d == rapp.d && w < rapp.w); }
	...
	public:
		...
		public:
			...
			double w(double S) const { return d / S + e; }
...
//		void update(uint64_t length, Time deadline, Time horizon);
	...
	private:
		double calculateStretch() { return stretch = (k * a + it->second.c) / (w * it->second.a + it->second.b); }
	...
	static void setNumPieces(unsigned int n) { numPieces = n; }

	const ClusteringVector<MDHCluster> & getSummary() const { return summary; }
	 ...
	 */
	double getMinimumStretch() const { return minimumStretch; }

	/**
	 * Returns the current maximum stretch for this set of nodes
	 */
	double getMaximumStretch() const { return maximumStretch; }
	...
	virtual StretchInformation * clone() const { return new StretchInformation(*this); }

	// This is documented in BasicMsg
	virtual void output(std::ostream& os) const;

	// This is documented in BasicMsg
	virtual std::string getName() const { return std::string("StretchInformation"); }
