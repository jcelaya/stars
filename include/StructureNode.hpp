...
public:
	 ...
	 */
	bool isAddition() const { return changing && actualLink == CommAddress() && newLink != CommAddress(); }

	/**
	 * Returns whether this object is changing and the zone is being deleted.
	 * @return True if it is.
	 */
	bool isDeletion() const { return changing && actualLink != CommAddress() && newLink == CommAddress(); }
	 ...
	 */
	bool testAndSet(uint64_t s) { if (seq < s) { seq = s; return true; } else return false; }
	 ...
	 */
	const CommAddress & getLink() const { return actualLink; }
	 ...
	 * @return The address of the link.
	 */
	const CommAddress & getNewLink() const { return newLink; }
	 ...
	 */
	void setLink(const CommAddress & addr) { newLink = addr; changing = true; }
	void resetLink() { newLink = CommAddress(); changing = true; }

	/**
	 * Returns the zone described in this object.
	 * @return This zone.
	 */
	const boost::shared_ptr<ZoneDescription> & getZone() const { return actualZone; }

	/**
	 * Sets the zone described in this object.
	 * @param i The new zone.
	 */
	void setZone(const boost::shared_ptr<ZoneDescription> & i) { newZone = i; if (!changing) actualZone = i; }
...
public:
	 ...
	 */
	bool isRNChildren() const { return level == 0; }

	bool inNetwork() const { return state == ONLINE; }

	int getLevel() const { return level; }

	const CommAddress & getFather() const { return father; }

	// Debug/Simulation methods

	boost::shared_ptr<const ZoneDescription> getZoneDesc() const { return zoneDesc; }

	boost::shared_ptr<const ZoneDescription> getNotifiedZoneDesc() const { return notifiedZoneDesc; }
	...
	boost::shared_ptr<TransactionalZoneDescription> getSubZone(int i) const {
		...
		if (it != subZones.end()) return *it; else return boost::shared_ptr<TransactionalZoneDescription>();
	}
	unsigned int getNumChildren() const { return subZones.size(); }
