...
public:
    ...
    void beforeEvent(const Simulator::Event & ev) {
        if (typeid(*ev.msg) == typeid(DispatchCommandMsg) && sim.getNode(ev.to).getSub().isIdle())
            workingNodes++;
    }
    ...
    void afterEvent(const Simulator::Event & ev) {
        if (sim.emptyEventQueue()) {
            ...
            if (SimTask::getRunningTasks() == 0) finish = true;
        }
    }

    bool blockMessage(uint32_t src, uint32_t dst, const shared_ptr<BasicMsg> & msg) {
        return dynamic_pointer_cast<AvailabilityInformation>(msg).get() != NULL;
    }
