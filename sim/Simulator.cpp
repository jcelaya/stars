...
void Simulator::log(tracked_string * category, int priority, const list<LogMsg::AbstractTypeContainer *> & values) {
    if (debugArchive.get()) {
        Category & cat = Category::getInstance(((string) * category));
        if (cat.isPriorityEnabled(priority)) {
            Duration realTime(getRealTime().total_microseconds());
            Time curTime = time;
            size_t numValues = values.size();
            CommAddress localAddr = CommAddress(0, 1234);
            if (currentNode != NULL)
                (*debugArchive) << realTime << curTime << priority << category << currentNode->getLocalAddress() << numValues;
            else
                (*debugArchive) << realTime << curTime << priority << category << localAddr << numValues;
            for (list<LogMsg::AbstractTypeContainer *>::const_iterator it = values.begin(); it != values.end(); it++) {
                const LogMsg::AbstractTypeContainer * p = (*it)->getSerializableVersion();
                (*debugArchive) << p;
                if (p != *it)
                    delete p;
            }
...
void Simulator::progressLog(const list<LogMsg::AbstractTypeContainer *> & values) {
    ostringstream oss;
    for (list<LogMsg::AbstractTypeContainer *>::const_iterator it = values.begin(); it != values.end(); ++it)
        oss << **it;

    cout << '#' << getpid() << ": " << oss.str() << endl;

    if (progressFile.is_open()) {
        progressFile << oss.str() << endl;
    }
}
...
void Simulator::setProperties(Properties & property) {
    ...
    if (debugFile.is_open())
        debugArchive.reset(new portable_binary_oarchive(debugFile));
    initLog(property("log_conf_string", string("")));
