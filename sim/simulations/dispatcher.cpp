...
//   saveDb(TaskBagAppDatabase::getInstance().getDatabase().getDatabase(), (statDir / fs::path("PeerComp.db")).c_str());
    }
};
...
class repeat : public SimulationCase {
    struct AppInstance {
        ...
        bool operator<(const AppInstance & r) const {
            return appNum < r.appNum;
        }
    };
...
//  fs::path statDir(property("results_dir", std::string("./results")));
//  if (property("memory_db", true))
//   saveDb(TaskBagAppDatabase::getInstance().getDatabase().getDatabase(), (statDir / fs::path("PeerComp.db")).file_string().c_str());
    }
};
