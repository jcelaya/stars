...
void TransactionalZoneDescription::commit() {
	if (changing) {
		actualLink = newLink; newLink = CommAddress();
		actualZone = newZone; newZone.reset();
		changing = false;
	}
}
...
std::ostream& operator<<(std::ostream& os, const TransactionalZoneDescription & s) {
	if (!s.changing) {
		os << "c=" << s.actualLink;
		os << " seq=" << s.seq << " ";
		if (s.actualZone.get()) os << *s.actualZone; else os << "?";
	} else {
		os << "c=" << s.actualLink;
		os << "/" << s.newLink;
		os << " seq=" << s.seq << " ";
		if (s.actualZone.get()) os << *s.actualZone; else os << "?";
		os << "/";
		if (s.newZone.get()) os << *s.newZone; else os << "?";
	}
	return os;
}
...
std::ostream & operator<<(std::ostream& os, const StructureNode & s) {
	...
	os << " f=" << s.father;
	os << " seq=" << s.seq << " ";
	if (s.zoneDesc.get()) os << *s.zoneDesc; else os << "?";
		...
		double distance[numChildren][numChildren];
		{
			...
			for (zoneConstIterator it = subZones.begin(); it != subZones.end(); it++, i++) {
				...
				for (zoneConstIterator jt = subZones.begin(); jt != it; jt++, j++) {
					...
					if (distance[i][j] > maxDist) { maxi = i; maxDist = distance[i][j]; }
				}
				distance[i][i] = 0.0;
			}
		}
