...
public:
	...
	HeartbeatTimeout * clone() const { return new HeartbeatTimeout(*this); }

	const CommAddress & getExecutionNode() const { return executionNode; }

	// This is documented in BasicMsg
	std::string getName() const { return std::string("HeartbeatTimeout"); }
};
...
void SubmissionNode::sendRequest(long int appInstance, int prevRetries) {
	if (!inChange) {
		...
		/*timeouts[rt->getRequestId()] = */CommLayer::getInstance().setTimer(timeout, rt);
	} else {
