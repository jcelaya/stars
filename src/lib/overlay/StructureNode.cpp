...
void TransactionalZoneDescription::setZoneFrom(const CommAddress & src, const std::shared_ptr<ZoneDescription> & i) {
...
static bool compareZones(const std::shared_ptr<TransactionalZoneDescription> & l, const std::shared_ptr<TransactionalZoneDescription> & r) {
...
void StructureNode::checkFanout() {
    ...
    if (transaction == NULL_TRANSACTION_ID && subZones.front()->getZone().get()) {
        if (subZones.size() >= 2*m) {
            ...
            std::shared_ptr<StrNodeNeededMsg> snnm(new StrNodeNeededMsg);
...
void StructureNode::recomputeZone() {
    ...
    list<std::shared_ptr<ZoneDescription> > tmp;
    ...
    if (tmp.empty())
        ...
        list<std::shared_ptr<ZoneDescription> >::iterator i = tmp.begin();
...
template<> void StructureNode::handle(const CommAddress & src, const InsertMsg & msg, bool self) {
    ...
    if (state == ONLINE) {
        ...
        if (father != CommAddress() && (src != father || self) && !zoneDesc->contains(msg.getWho())) {
            ...
            txDriver = msg.getWho();
            std::shared_ptr<TransactionalZoneDescription> newZone(new TransactionalZoneDescription);
...
template<> void StructureNode::handle(const CommAddress & src, const UpdateZoneMsg & msg, bool self) {
    ...
    for (zoneMutableIterator it = subZones.begin(); it != subZones.end(); it++, i++) {
        if ((*it)->comesFrom(src)) {
            ...
            (*it)->setZoneFrom(src, std::shared_ptr<ZoneDescription>(new ZoneDescription(msg.getZone())));
            subZones.sort(compareZones);
...
template<> void StructureNode::handle(const CommAddress & src, const NewStrNodeMsg & msg, bool self) {
    ...
    if (transaction == msg.getTransactionId() && state == WAIT_STR) {
        ...
        std::shared_ptr<TransactionalZoneDescription> zone[numChildren];
        {
        ...
        for (zoneMutableIterator it = subZones.begin(); it != subZones.end(); it++, i++) {
            if (src == (*it)->getLink()) {
                ...
                if (msg.replaces()) {
                    ...
                    (*it)->setZone(std::shared_ptr<ZoneDescription>());
                    subZones.sort(compareZones);
                } else {
