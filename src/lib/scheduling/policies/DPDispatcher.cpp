...
struct DPDispatcher::DecissionInfo {
    ...
    DecissionInfo(const DPAvailabilityInformation::AssignmentInfo & c, unsigned int b, double d)
...
void DPDispatcher::handle(const CommAddress & src, const TaskBagMsg & msg) {
    ...
    vector<list<DPAvailabilityInformation::AssignmentInfo> > finalAssignment(children.size());
