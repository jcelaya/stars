...
template<class T> class AggregationTest {
	...
	shared_ptr<T> newNode() {
		if (nextNode != nodes.end()) {
			...
			totalPower += n.power; totalMem += n.mem; totalDisk += n.disk;
...
public:
	...
	unsigned int getMinSize() const { return minSize; }

	unsigned int getMaxSize() const { return maxSize; }

	double getMeanSize() const { return (double)bytes / messages; }

	unsigned long int getTotalPower() const { return totalPower; }

	unsigned long int getTotalMem() const { return totalMem; }

	unsigned long int getTotalDisk() const { return totalDisk; }

	unsigned long int getNumNodes() const { return nodes.size(); }

	Priv<T> & getPrivateData() { return privateData; }
